...
int main(void)
{
  ...
  KeyPadTaskHandle = osThreadNew(KeyPad, NULL, &KeyPadTask_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  //HAL_GPIO_WritePin(GPIOA, KP_R1_Pin|GPIO_Fertilizer_Pin|GPIO_Water_Pin|KP_R2_Pin|GPIO_OP_Buzzer_Pin|KP_R3_Pin|KP_R4_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : KP_C3_Pin KP_C4_Pin */
  GPIO_InitStruct.Pin = KP_C3_Pin|KP_C4_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pins : KP_C1_Pin KP_C2_Pin */
  GPIO_InitStruct.Pin = KP_C1_Pin|KP_C2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
...
typedef enum {
    VIEW_WATER,
    SETTINGS_SOIL_THRESHOLD,
    SETTINGS_TANK_THRESHOLD
} DisplayState_t;

// Shared threshold values � read by Watering task, written by display task
volatile uint16_t g_soilDryThreshold  = 2500;  // matches your current #define
volatile uint16_t g_tankEmptyThreshold = 750;   // matches your current #define

// Latest sensor readings updated by Watering task
...
void StartWatering(void *argument)
{
  /* USER CODE BEGIN StartWatering */
	  HAL_UART_Transmit(&huart2, (uint8_t*)"Watering task started\r\n", 23, 100);
...
//  osDelay(2500);
  ...
  for(;;)
  {
    ...
    if (!wateringAllowed) {
        LCD_Print("Cooldown...     ");
    } else if (soilvalue > g_soilDryThreshold ) {
    ...
    osMutexRelease(lcdMutexHandle);
		...
		osDelay(1000);
		if (waterLevel < g_tankEmptyThreshold ) {
			tankEmpty = 1;
			osMutexAcquire(lcdMutexHandle, osWaitForever);
			LCD_SetCursor(1, 0);
        ...
        tankEmpty = 0; // clears automatically when tank is refilled
			...
			// -- watering logic --
			if (soilvalue > g_soilDryThreshold  && wateringAllowed && !tankEmpty) {
				{
...
void KeyPad(void *argument)
{
  /* USER CODE BEGIN KeyPad */
	 HAL_UART_Transmit(&huart2, (uint8_t*)"Keypad task started\r\n", 21, 100);
  ...
  for(;;)
  {
    ...
    {
        int len = snprintf(msg, sizeof(msg), "Key pressed: %c\r\n", key);
